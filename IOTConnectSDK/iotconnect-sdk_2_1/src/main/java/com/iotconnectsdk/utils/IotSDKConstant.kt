package com.iotconnectsdk.utils

internal object IotSDKConstant {
    const val CORE_COMMAND = "0x01"
    const val FIRMWARE_UPDATE = "0x02"
    const val ATTRIBUTE_INFO_UPDATE = "0x10"
    const val SETTING_INFO_UPDATE = "0x11"
    const val PASSWORD_INFO_UPDATE = "0x12"
    const val DEVICE_INFO_UPDATE = "0x13"
    const val RULE_INFO_UPDATE = "0x15"

    const val STOP_SDK_CONNECTION = "0x99"
    const val M_ANDROID = "M_android"
}